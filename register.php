...
if(isset($_POST['submit'])){
   ...
   $select_user = $conn->prepare("SELECT * FROM users WHERE email = ? OR date = ?");
   $select_user->execute([$email, $date]);
   ...
   if($select_user->rowCount() > 0){
      ...
      if($pass != $cpass){
         ...
         $insert_user = $conn->prepare("INSERT INTO users(name, email, date, password) VALUES(?,?,?,?)");
         $insert_user->execute([$name, $email, $date, $cpass]);
...
<section class="form-container">
   ...
   <form action="" method="post">
      ...
      <input type="date" name="date" required placeholder="masukkan tanggal lahir" class="box" min=""  maxlength="15">
